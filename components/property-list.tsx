      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {paginatedProperties.map((property) => (
            <Card key={property.id} className="overflow-hidden glass-card ios-hover">
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="col-span-2">
                    ...
                    <p className="font-bold text-lg text-success">{formatPrice(property.price)}</p>
                  </div>
                  <div>
                ...
                <div className="flex gap-2 pt-2">
                  <Link href={`/objects/${property.id}`} className="flex-1">
                    <Button variant="outline" size="sm" className="w-full glass-card bg-transparent">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleDelete(property.id)} className="glass-card">
        ...
        <div className="text-sm text-muted-foreground text-center">
          Показано {startIndex + 1}–{Math.min(startIndex + itemsPerPage, sortedProperties.length)} з{" "}
          {sortedProperties.length} об'єктів
        </div>
      </div>
    )
  }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right text-success font-semibold">Ціна</TableHead>
          ...
          <TableBody>
            {paginatedProperties.map((property) => (
              <TableRow key={property.id}>
                ...
                <TableCell className="text-right font-bold text-success">{formatPrice(property.price)}</TableCell>
